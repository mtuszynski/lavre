...
function lavre_category_category()
{
    $product_categories = get_terms(array(
        ...
        'order'   => 'ASC',
        'hide_empty' => true,
        'number' => 5, // Ogranicz do 5 kategorii
    ));

    if (!empty($product_categories) && !is_wp_error($product_categories)) { ?>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 py-16 clear-both">
            <?php
            ...
            ?>
                <div class="flex flex-col gap-8 items-center group">
                    <?php if ($image_url) { ?>
                        <a href="<?php echo esc_url(get_term_link($category)); ?>"><img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($category->name); ?>" class="w-full object-cover rounded-full aspect-square transition-all group-hover:brightness-125"></a>
                    <?php } else {
                    ?>
                        <a href="<?php echo esc_url(get_term_link($category)); ?>" alt="<?php echo esc_attr($category->name); ?>" class="w-full bg-light rounded-full aspect-square flex items-center justify-center group"></a>
                    <?php } ?>
